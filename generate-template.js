...
// 函数一：下载所有卡片
// ===================================================================================
async function downloadAllCards(page) {
  ...
  for (const card of cards) {
    ...
    const fileName = `card-${cardIndex}.png`;
    const filePath = path.join(outputDir, fileName);

    console.log(`  -> 正在截图卡片 #${cardIndex}...`);
...
(async () => {
  ...
  try {
    ...
    const filePath = `file://${path.join(__dirname, 'html/ooooaad.html')}`;
    await page.goto(filePath, { waitUntil: 'networkidle0' });

    // --- 在这里选择你要执行的任务 ---
    // 选项 A: 下载所有卡片
    await downloadAllCards(page);
